...
const gateDefinitions = {
  'runtime-hostile': {
    ...
    observed: 'QA diagnostic: the deterministic Level 1 hostile matrix completed input, collision, HUD, and terminal-state probes.', normal: false,
  },
  'campaign-progression': {
    ...
    observed: 'QA diagnostic: forced terminal-state probes advanced through the ordered release and recorded campaign-owned state.', normal: false,
  },
  'boarding-entry-abort': {
    ...
    observed: 'QA diagnostic: deterministic Boarding probes recorded entry, touch-combat, pause, and abort return behaviour.', normal: false,
  },
  'boarding-success-return': {
    ...
    observed: 'QA diagnostic: deterministic Boarding probes recorded success, physical exit, and Shooter return behaviour.', normal: false,
  },
  'level4-hazards': {
    ...
    observed: 'QA diagnostic: Level 4 hazard probes recorded configured hazard and projectile resolution behaviour.', normal: false,
  },
  'designer-roundtrip': {
...
const gates = REQUIRED_GATES.map((id) => {
  ...
  return {
    id,
    classification: /(?:[?&]qa=|forceComplete|forceFail)/i.test(`${definition.route} ${definition.actions.join(' ')}`) ? 'QA_DIAGNOSTIC' : 'AUTOMATED_BROWSER',
