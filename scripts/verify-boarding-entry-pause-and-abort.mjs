...
async function clickAction(page, action) {
  ...
  const target = snapshot.terminalActions.find(
...
async function reachLevelFour(page) {
  for (let sequence = 1; sequence < 4; sequence += 1) {
    ...
    await page.waitForFunction((next) => {
      ...
      return (
        value?.terminalState === null && value?.campaign?.sequence === next
      );
    }, sequence + 1);
  }
}
...
try {
  ...
  assert(
    levelFour.campaign.sequence === 4,
    "Did not reach the distinct Level 4 campaign definition.",
  );
  assert(
    levelFour.activeScouts > 0,
    "Level 4 has no active Boarding anchor candidate.",
  );
  await page.waitForFunction(
  ...
  try {
    await page.waitForFunction(
      () => Boolean(window.__GALACTIC_GUNNERS_BOARDING_QA__),
      undefined,
      { timeout: 15_000 },
    );
  } catch (error) {
    console.error(
      "Boarding launch diagnostic:",
      JSON.stringify(
        await page.evaluate(() => ({
          hostile: window.__GALACTIC_GUNNERS_HOSTILE__?.state(),
          runtimeGlobals: Object.keys(window).filter((key) =>
            key.includes("GALACTIC") || key.includes("PHASER"),
          ),
        })),
      ),
    );
    console.error("Boarding browser errors:", JSON.stringify(pageErrors));
    console.error("Boarding console errors:", JSON.stringify(consoleErrors));
    throw error;
  }
  try {
    await page.waitForFunction(
      () => {
        ...
        return Boolean(
          value?.active && value?.serverRunId && !value?.serverError,
        );
      },
      undefined,
      { timeout: 15_000 },
    ...
    console.error(
      "Boarding activation diagnostic:",
      JSON.stringify(
        await page.evaluate(() =>
          window.__GALACTIC_GUNNERS_BOARDING_QA__?.state(),
        ),
      ),
    );
    throw error;
  ...
  assert(
    active.active,
    `Boarding did not become active: ${JSON.stringify(active)}`,
  );
  assert(
    active.serverRunId,
    "Boarding did not receive a server-authoritative run.",
  );
  assert(!active.serverError, `Boarding server error: ${active.serverError}`);
  assert(
    active.touchControls.length === 5,
    `Expected five touch controls, received ${JSON.stringify(active.touchControls)}`,
  );
  assert(
    active.playerBody.width >= 30 && active.playerBody.height >= 78,
    `Boarding player collider is not aligned to the rendered envelope: ${JSON.stringify(active.playerBody)}`,
  );
  assert(
    active.alienBodies.length === 6 &&
      active.alienBodies.every(
        (alien) => alien.body.width >= 34 && alien.body.height >= 72,
      ),
    `Boarding alien colliders are not aligned to their rendered envelopes: ${JSON.stringify(active.alienBodies)}`,
  );
  await page.screenshot({
    path: path.join(outputDir, "01-boarding-active.png"),
    fullPage: true,
  });

  // Pause is an ordinary-player interaction. Exercise it before holding the
  // player in live combat for the independent touch-fire assertion below.
  ...
  try {
    ...
    console.error(
      "Boarding pause diagnostic:",
      JSON.stringify(
        await page.evaluate(() => ({
          boarding: window.__GALACTIC_GUNNERS_BOARDING_QA__?.state(),
          hostile: window.__GALACTIC_GUNNERS_HOSTILE__?.state(),
        })),
      ),
    );
  ...
  await page.screenshot({
    path: path.join(outputDir, "02-boarding-pause.png"),
    fullPage: true,
  });
  ...
  const fire = active.touchControls.find(
    (control) => control.id === "boarding-touch-fire",
  );
  ...
  await touch.send("Input.dispatchTouchEvent", {
    type: "touchStart",
    touchPoints: [
      { x: fireX, y: fireY, id: 1, radiusX: 1, radiusY: 1, force: 1 },
    ],
  });
  try {
    await page.waitForFunction(
    ...
    await page.waitForFunction(
      () =>
        window.__GALACTIC_GUNNERS_BOARDING_QA__?.state()?.playerShotsFired > 0,
      undefined,
      { timeout: 15_000 },
    );
  } finally {
  ...
  const afterTouchFire = await page.evaluate(() =>
    window.__GALACTIC_GUNNERS_BOARDING_QA__?.state(),
  );
  assert(
    afterTouchFire.playerShotsFired > 0,
    `Touch Fire did not activate a projectile: ${JSON.stringify(afterTouchFire)}`,
  );
  await page.screenshot({
    path: path.join(outputDir, "01a-boarding-touch-fire.png"),
    fullPage: true,
  });
  ...
  assert(
    returned.terminalState === null,
    "Boarding abort did not return to active Shooter gameplay.",
  );
  await page.screenshot({
    path: path.join(outputDir, "03-boarding-abort-return.png"),
    fullPage: true,
  });
  ...
  writeFileSync(
    path.join(outputDir, "boarding-browser-verification.json"),
