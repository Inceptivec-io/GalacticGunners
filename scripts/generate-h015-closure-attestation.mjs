...
import {
  ...
  REQUIRED_GATES,
  sha256,
} from "./verify-h015-evidence-integrity.mjs";
...
const outputDirectory = path.resolve(
  process.env.GG_CLOSURE_ATTESTATION_DIR ??
    path.join(root, "..", "h015-closure-attestation"),
);
...
const failures = auditManifest(manifest, {
  ...
  allowPendingClosureGate: true,
});
const closureGate = manifest.gates?.find((gate) => gate.id === "closure-audit");
if (
  !closureGate ||
  closureGate.result !== "PENDING" ||
  (closureGate.evidence ?? []).length !== 0
) {
  failures.push(
    "pre-attestation evidence manifest must contain a PENDING closure-audit gate with no closure evidence.",
  );
}
const requiredGates =
  manifest.gates?.filter((gate) => gate.id !== "closure-audit") ?? [];
if (requiredGates.length !== REQUIRED_GATES.length - 1)
  failures.push(
    "pre-attestation manifest does not contain every substantive required gate.",
  );
if (failures.length)
  throw new Error(
    `H015 pre-attestation audit failed:\n${failures.map((failure) => `- ${failure}`).join("\n")}`,
  );

const gateResults = manifest.gates.map((gate) => ({
  id: gate.id,
  result: gate.id === "closure-audit" ? "PASS" : gate.result,
}));
const failedGates = gateResults.filter((gate) => gate.result === "FAIL").length;
const pendingGates = gateResults.filter(
  (gate) => gate.result === "PENDING",
).length;
if (failedGates !== 0 || pendingGates !== 0)
  throw new Error(
    `H015 attestation cannot close with failed=${failedGates} pending=${pendingGates}.`,
  );
...
const closureAudit = {
  ...
  audited_evidence_digest: evidenceDigest(manifest.gates),
  ...
  auditor: {
    ...
    mode: "fail-closed substantive-gate audit over immutable uploaded evidence manifest",
  },
};
const closureAuditPath = path.join(
  outputDirectory,
  "closure-audit-result.json",
);
writeFileSync(closureAuditPath, `${JSON.stringify(closureAudit, null, 2)}\n`);
const attestation = {
  schema_version: "1.0",
  ...
  closure_audit_evidence: {
    path: path.basename(closureAuditPath),
    sha256: sha256(closureAuditPath),
  },
  required_gates: gateResults,
  evidence_uniqueness: "PASS",
  closure_audit: closureAudit.result,
  failed_gates: failedGates,
  pending_gates: pendingGates,
  auditor: {
    command: "node scripts/generate-h015-closure-attestation.mjs",
