...
try {
  await page.goto(`${baseUrl}/play`, { waitUntil: 'domcontentloaded', timeout: 15_000 });
  ...
  assert(menu?.heroKeyArt === 'GG-KEYART-KEY-ART-POSTERS-GG-HERO-IMAGE-PLAYER-FIGHTING-V002-4K-UHD-MASTER', 'Main menu does not use approved hero key art after splash.');
  const focusedCanvas = await page.evaluate(() => document.activeElement?.tagName === 'CANVAS');
  assert(focusedCanvas, 'Splash completion did not restore keyboard focus to the game canvas.');
  ...
  await page.waitForFunction(() => window.__GALACTIC_GUNNERS_MENU_QA__?.scene === 'MainMenuScene', { timeout: 8_000 });
  await page.locator('canvas').focus();
  await page.keyboard.press('Enter');
  await page.waitForFunction(() => window.__GALACTIC_GUNNERS_SLICE_QA__?.scene === 'Level1Scene', { timeout: 8_000 });
  ...
  assert(Math.abs(frozen.playerX - beforePause.playerX) <= 1
    && Math.abs(frozen.playerY - beforePause.playerY) <= 1
    && frozen.score === beforePause.score
    && frozen.lives === beforePause.lives
    && frozen.formationDropY === beforePause.formationDropY, 'Gameplay changed while paused.');
  ...
  await page.waitForFunction(() => window.__GALACTIC_GUNNERS_MENU_QA__?.scene === 'MainMenuScene', { timeout: 5_000 });
  assert(!await page.evaluate(() => Boolean(window.__GALACTIC_GUNNERS_PAUSE_QA__)), 'Pause surface remained after Main Menu navigation.');
  await page.waitForTimeout(2_300);
  assert(await page.evaluate(() => window.__GALACTIC_GUNNERS_MENU_QA__?.scene === 'MainMenuScene'), 'Splash reappeared during internal navigation.');
  ...
  writeFileSync(path.join(outputDir, 'stage-2-navigation-verification.json'), `${JSON.stringify(result, null, 2)}\n`);
