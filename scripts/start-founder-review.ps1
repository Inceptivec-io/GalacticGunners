...
try {
  ...
  Invoke-EvidenceRuntime 'shooter-baseline' { npm run runtime:shooter-baseline } 'Shooter baseline verification'
  Invoke-EvidenceRuntime 'navigation-and-pause' { npm run runtime:navigation-and-pause } 'splash and pause navigation verification'
  Invoke-EvidenceRuntime 'designer-pointer-interaction' { npm run runtime:designer-pointer-interaction } 'Designer pointer verification'
  Invoke-EvidenceRuntime 'designer-authoring-persistence' { npm run runtime:designer-authoring-persistence } 'Designer authoring verification'
  Invoke-EvidenceRuntime 'authentication-session' { npm run runtime:authentication-session } 'same-origin authentication and logout verification'
  Invoke-EvidenceRuntime 'campaign_runtime' { npm run runtime:campaign } 'campaign continuity verification'
  Invoke-EvidenceRuntime 'hazard-direction-rotation-and-destruction' { npm run runtime:hazard-direction-rotation-and-destruction } 'hazard verification'
  Invoke-EvidenceRuntime 'boarding-entry-pause-and-abort' { npm run runtime:boarding-entry-pause-and-abort } 'Boarding entry, pause, touch, and abort verification'
  Invoke-EvidenceRuntime 'boarding-combat-exit-and-return' { npm run runtime:boarding-combat-exit-and-return } 'Boarding combat, physical exit, and server-return verification'
  Invoke-EvidenceRuntime 'designer-save-preview-publish' { npm run runtime:designer-save-preview-publish } 'Designer draft, exact-checksum preview, publication, and runtime verification'
  Invoke-EvidenceRuntime 'founder-product-review-matrix' { npm run runtime:founder-product-review-matrix } 'browser review matrix verification'
