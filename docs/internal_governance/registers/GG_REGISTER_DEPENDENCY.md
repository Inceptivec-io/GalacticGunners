...
| playwright | package-lock pinned dev set | npm registry / `package-lock.json` | Handoff 010 browser runtime verifier for `/play` Phaser slice, screenshots, console/network checks and collision proof | Apache-2.0 | NO runtime notice; dev/test only | CURRENT | LOW |
