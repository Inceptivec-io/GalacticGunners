...
test('H015-MENU-001__e2e_ordinary_user__keyboard_confirmation_starts_gameplay_once', async ({ page, strictRuntime }) => {
...
test('H015-MENU-001__e2e_ordinary_user__mouse_selects_visible_start_control', async ({ page, strictRuntime }) => {
  await waitForMenu(page);
...
test('H015-MENU-001__e2e_ordinary_user__touch_selects_visible_start_control', async ({ page, strictRuntime }) => {
  test.skip(!('hasTouch' in test.info().project.use) || !test.info().project.use.hasTouch, 'Touch behaviour runs in the mobile project.');
  await waitForMenu(page);
  await clickStartControl(page);
  await expect(page.locator('[data-game-status]')).toHaveText('Galactic Gunners gameplay started.');
  expect(strictRuntime.unexpectedFailures).toEqual([]);
...
test('H015-MENU-001__e2e_ordinary_user_negative__duplicate_mouse_activation_does_not_restart_or_error', async ({ page, strictRuntime }) => {
  await waitForMenu(page);
  await clickStartControl(page);
  await clickStartControl(page);
  await expect(page.locator('[data-game-status]')).toHaveText('Galactic Gunners gameplay started.');
