...
test("CI readiness accepts an HTTP 200 response", async () => {
  ...
  try {
    const result = await runWaitForHttp(["--url", `http://127.0.0.1:${port}`, "--timeout-ms", "100", "--interval-ms", "10", "--request-timeout-ms", "20"]);
...
test("CI readiness fails closed when no server can respond", async () => {
  ...
  const result = await runWaitForHttp(["--url", `http://127.0.0.1:${port}`, "--timeout-ms", "50", "--interval-ms", "5", "--request-timeout-ms", "10"]);
