...
export interface LevelAuthoringDocument {
  ...
  hazard_emitters: Array<{ id: string; hazard_type: 'ASTEROID' | 'COMET'; asset_id: string; variant_mode?: 'FIXED' | 'ORDERED' | 'SEEDED_RANDOM'; variant_ids?: string[]; enabled: boolean; initial_count: number; maximum_active: number; spawn_interval_ms: number; spawn_jitter_ms: number; speed_min: number; speed_max: number; angular_velocity_min: number; angular_velocity_max: number; spin_direction_policy?: 'SEEDED_BIDIRECTIONAL' | 'CLOCKWISE' | 'COUNTERCLOCKWISE' | 'NONE'; entry_edges: Array<'TOP' | 'RIGHT' | 'BOTTOM' | 'LEFT'>; spawn_pattern: 'RANDOM_EDGE' | 'ALTERNATING_EDGES' | 'LANE' | 'FIXED_POINTS'; spawn_points: Array<{ x: number; y: number }>; despawn_margin: number; collision_damage: number }>;
